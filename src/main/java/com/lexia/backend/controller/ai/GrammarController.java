 ...
 * Provides endpoints for generating, submitting, and tracking grammar
 * exercises.
 * 
 * <p>
 * Base path: /api/v1/ai/grammar
 * </p>
 * 
 * <p>
 * Features:
...
public class GrammarController {
         ...
         * 
         * <p>
         * Uses Google Gemini AI to generate grammar exercises based on the specified
         * topic, CEFR level, and optional theme. If AI generation fails, fallback
         * content is used.
         * </p>
         * 
         * @param user    the authenticated user
         * @param request the generation request containing grammar topic, CEFR level,
         *                etc.
        ...
        @Operation(summary = "Generate grammar exercises", description = "Uses AI to generate grammar exercises based on topic, CEFR level, and theme. "
                        +
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Exercises generated successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = GrammarExerciseSetDTO.class), examples = @ExampleObject(name = "Generated Exercises", value = """
                                        {
                        ...
                        @ApiResponse(responseCode = "429", description = "Rate limit exceeded - daily grammar quota reached (50/day)", content = @Content(mediaType = "application/json"))
        })
        @QuotaCheck(contentType = "grammar", incrementSession = true, sessionType = "grammar")
        ...
        @Operation(summary = "List grammar topics", description = "Returns all available grammar topics for exercise generation. "
                        +
                        "Topics are organized by category (Tenses, Modals, etc.) and tagged with applicable CEFR levels.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Topics retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = GrammarTopicDTO.class), examples = @ExampleObject(name = "Topics List", value = """
                                        [
                                          {
                                            "id": 1,
                                            "name": "Present Simple",
        ...
        @Operation(summary = "List topics by CEFR level", description = "Returns grammar topics applicable to the specified CEFR level (A1-C2).")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<GrammarTopicDTO>> getTopicsByLevel(
                        @Parameter(description = "CEFR level (A1, A2, B1, B2, C1, C2)", example = "B1") @PathVariable String level) {
        ...
        @Operation(summary = "List topics by category", description = "Returns grammar topics in the specified category (Tenses, Modals, etc.).")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Topics retrieved successfully"),
                        @ApiResponse(responseCode = "401", description = "Unauthorized")
        })
        public ResponseEntity<List<GrammarTopicDTO>> getTopicsByCategory(
                        @Parameter(description = "Category name", example = "Tenses") @PathVariable String category) {
        ...
        @Operation(summary = "List topic categories", description = "Returns all distinct grammar topic categories.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Get exercise set", description = "Retrieves a complete exercise set by ID, including all exercises and the explanation.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Exercise set retrieved successfully", content = @Content(schema = @Schema(implementation = GrammarExerciseSetDTO.class))),
                        @ApiResponse(responseCode = "401", description = "Unauthorized"),
        ...
        public ResponseEntity<GrammarExerciseSetDTO> getExerciseSet(
                        @Parameter(description = "Exercise set UUID") @PathVariable UUID id) {
        ...
        @Operation(summary = "List user exercise sets", description = "Returns paginated list of exercise sets generated for the authenticated user.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<Page<GrammarExerciseSetDTO>> getUserExerciseSets(
                ...
                Page<GrammarExerciseSetDTO> exerciseSets = grammarExerciseService.getUserExerciseSets(user.getId(),
                                pageable);
        ...
        @Operation(summary = "Submit exercise answers", description = "Submits answers for an exercise set and returns detailed results. "
                        +
                        "Calculates score, provides feedback per question, and tracks progress. " +
                        "Passing threshold is 70%.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Answers submitted and scored successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = GrammarResultDTO.class), examples = @ExampleObject(name = "Result", value = """
                                        {
        ...
        public ResponseEntity<GrammarResultDTO> submitAnswers(
                        @AuthenticationPrincipal User user,
                        @Parameter(description = "Exercise set UUID") @PathVariable UUID exerciseSetId,
        ...
        @Operation(summary = "Check if submitted", description = "Checks if the user has already submitted answers for an exercise set.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Get exercise history", description = "Returns paginated history of completed exercise sets with scores.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Get user statistics", description = "Returns aggregated statistics including total attempts, pass rate, "
                        +
                        "average score, topic breakdown, and streak information.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Statistics retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = GrammarStatsDTO.class), examples = @ExampleObject(name = "Stats", value = """
                                        {
        ...
        @Operation(summary = "Get exercise progress", description = "Retrieves the user's progress for a specific exercise set.")
        @ApiResponses(value = {
         ...
         */
        @DeleteMapping(value = "/exercises/{exerciseSetId}/progress")
        @Operation(summary = "Reset exercise progress", description = "Deletes the user's progress for an exercise set, allowing them to retry the exercise.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "204", description = "Progress reset successfully"),
                        @ApiResponse(responseCode = "401", description = "Unauthorized"),
                        @ApiResponse(responseCode = "404", description = "Exercise set or progress not found")
        })
        public ResponseEntity<Void> resetProgress(
                        @AuthenticationPrincipal User user,
                        @Parameter(description = "Exercise set UUID") @PathVariable UUID exerciseSetId) {

                LOG.info("User {} resetting progress for exercise set {}", user.getEmail(), exerciseSetId);
                grammarExerciseService.resetProgress(exerciseSetId, user.getId());
        ...
        @Operation(summary = "Get fallback exercises", description = "Returns pre-seeded fallback exercises for when AI is unavailable or explicitly requested.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<GrammarExerciseSetDTO> getFallbackExercises(
                        @Parameter(description = "Grammar topic", example = "Present Simple") @RequestParam String topic,
                        @Parameter(description = "CEFR level", example = "B1") @RequestParam String level) {
