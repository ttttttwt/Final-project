...
public class FlashcardController {
  ...
  public ResponseEntity<FlashcardStudySessionDTO> getStudySession(
      ...
      @Parameter(description = "Session mode: 'normal' for due cards, 'practice' for all cards") @RequestParam(required = false, defaultValue = "normal") String mode) {

    LOG.info("User {} starting study session for deck {} (mode={})", user.getEmail(), deckId, mode);

    // Cap maxCards at 50
    maxCards = Math.min(maxCards, 50);

    FlashcardStudySessionDTO session;
    if ("practice".equalsIgnoreCase(mode)) {
      session = flashcardService.getPracticeSession(deckId, user.getId(), maxCards);
    } else {
      session = flashcardService.getStudySession(deckId, user.getId(), maxCards);
    }

    LOG.info("User {} study session: {} cards to study ({} due, {} new)",
        user.getEmail(), session.getSessionSize(), session.getDueCards(),
        session.getNewCards());
