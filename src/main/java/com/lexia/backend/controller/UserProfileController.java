...
public class UserProfileController {
  ...
  @PreAuthorize("isAuthenticated()")
  @Operation(summary = "Upload or update user avatar (URL)", description = "Updates the avatar URL for the currently authenticated user. "
      +
      "Accepts avatar URL as JSON. For file upload, use POST /api/v1/users/avatar endpoint.", security = @SecurityRequirement(name = "Bearer Authentication"))
  @ApiResponses(value = {
   ...
   * @return ResponseEntity with updated profile containing new avatar URL
   */
  @PostMapping(value = "/avatar", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
  @PreAuthorize("isAuthenticated()")
  @Operation(summary = "Upload user avatar (file)", description = "Uploads an avatar image file for the currently authenticated user. "
      +
      "Accepts image files (JPG, PNG, GIF, WebP) up to 5MB. " +
      "Old avatar file will be deleted when a new one is uploaded.", security = @SecurityRequirement(name = "Bearer Authentication"))
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "Avatar uploaded successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserProfileDTO.class))),
      @ApiResponse(responseCode = "400", description = "Invalid file type or size", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))),
  ...
  public ResponseEntity<UserProfileDTO> uploadAvatarFile(
      @RequestParam("file") MultipartFile file) {
    LOG.info("Received request to upload avatar file");

    UserProfileDTO currentProfile = userProfileService.getCurrentUserProfile();
    UserProfileDTO updatedProfile = userProfileService.uploadAvatar(currentProfile.getUserId(), file);

    LOG.info("Successfully uploaded avatar for user: {}", currentProfile.getUserId());
