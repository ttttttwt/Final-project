...
public class AdminAIConfigController {
    ...
    public ResponseEntity<AIFeatureConfig> updateFeatureConfig(@PathVariable String featureName, @RequestBody AIFeatureConfig config) {
        return ResponseEntity.ok(configService.updateFeatureConfig(featureName, config));
    }

    @PatchMapping("/features/{featureName}/toggle")
    public ResponseEntity<AIFeatureConfig> toggleFeature(@PathVariable String featureName, @RequestBody java.util.Map<String, Boolean> body) {
