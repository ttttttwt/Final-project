...
public interface UserRepository extends JpaRepository<User, UUID>, JpaSpecificationExecutor<User> {
    ...
    long countUsersCreatedBetween(@Param("start") java.time.LocalDateTime start,
            @Param("end") java.time.LocalDateTime end);

    /**
     * Count active users (users who logged in after a date)
     */
    @Query("SELECT COUNT(DISTINCT u) FROM User u WHERE u.updatedAt >= :date AND u.isActive = true")
    long countActiveUsersAfter(@Param("date") java.time.LocalDateTime date);

    /**
     * Count active users between dates
     */
    @Query("SELECT COUNT(DISTINCT u) FROM User u WHERE u.updatedAt BETWEEN :start AND :end AND u.isActive = true")
    long countActiveUsersBetween(@Param("start") java.time.LocalDateTime start,
            @Param("end") java.time.LocalDateTime end);

    /**
     * Count free users (users without active subscription)
     */
    @Query("""
            SELECT COUNT(u) FROM User u
            WHERE NOT EXISTS (
                SELECT 1 FROM Subscription s WHERE s.user = u AND s.status = com.lexia.backend.enums.SubscriptionStatus.ACTIVE
            )
            """)
    long countFreeUsers();

    /**
     * Count users by CEFR level
     */
    @Query("SELECT COUNT(u) FROM User u JOIN u.profile p WHERE p.currentLevel = :level")
