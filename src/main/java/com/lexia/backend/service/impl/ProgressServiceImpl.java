...
public class ProgressServiceImpl implements ProgressService {
        ...
        public LessonProgressDTO completeLesson(User user, Long lessonId, String resultDetailsJson) {
                ...
                LessonProgress progress = lessonProgressRepository
                                ...
                                .orElseGet(() -> {
                                        log.debug("Creating new progress record for user {} and lesson {}",
                                                        user.getId(), lessonId);
                ...
                // Publish lesson completed event for notification
                publishLessonCompletedEvent(user.getId(), lesson);
        ...
        /**
         * Publishes a LessonCompletedEvent for notification system.
         *
         * @param userId the user who completed the lesson
         * @param lesson the completed lesson
         */
        private void publishLessonCompletedEvent(UUID userId, Lesson lesson) {
                try {
                        ...
                        log.debug("Published LessonCompletedEvent for user {} completing lesson {}",
                                        userId, lesson.getId());
                } catch (Exception e) {
                        // Don't fail lesson completion if notification fails
                        log.warn("Failed to publish LessonCompletedEvent for user {} in lesson {}: {}",
                                        userId, lesson.getId(), e.getMessage());
                }
        }
        ...
        public com.lexia.backend.dto.CourseProgressDTO getCourseProgress(User user, Long courseId) {
                ...
                com.lexia.backend.entity.Course course = courseRepository.findById(courseId)
                                .orElseThrow(() -> new com.lexia.backend.exception.ResourceNotFoundException(
                                                "Course not found with id: " + courseId));

                // Get all lesson progress for this course
                List<LessonProgress> lessonProgressList = lessonProgressRepository.findByUserIdAndCourseId(user.getId(),
                                courseId);
                ...
                // Map to DTO
                return com.lexia.backend.mapper.ProgressMapper.toCourseProgressDTO(course, lessonProgressList,
                                totalLessons);
        }
        ...
        public com.lexia.backend.dto.DashboardOverviewDTO getDashboardOverview(User user) {
                ...
                // Calculate total study time (mocked for now as we don't track time per session
                // yet)
                ...
                List<com.lexia.backend.dto.ActivityDTO> recentActivities = recentProgress.stream()
                                ...
                                .map(p -> com.lexia.backend.dto.ActivityDTO.builder()
                                                ...
                                                .link("/courses/" + p.getLesson().getSection().getCourse().getId()
                                                                + "/lessons/" + p.getLesson().getId())
        ...
        public com.lexia.backend.dto.ProgressSummaryDTO getProgressSummary(User user, int days) {
                ...
                for (int i = days - 1; i >= 0; i--) {
                        ...
                        List<LessonProgress> dailyProgress = lessonsByDate.getOrDefault(dateStr,
                                        Collections.emptyList());

                        int dailyLessons = dailyProgress.size();
                        // Estimate time spent: 15 mins per lesson (since we don't track actual time
                        // yet)
